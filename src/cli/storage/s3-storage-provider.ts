...
export const buildStoreProvider: StorageProviderBuilder = (
  ...
  const awsCredentialsResult = loadAwsCredentials({
    awsAccessKeyId: context.awsAccessKeyId,
    awsSecretAccessKey: context.awsSecretAccessKey,
  });
  if (awsCredentialsResult == null) {
    throw new Error("❌ S3 Credentials not provided.\nSet the AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables for a set of credentials that has write access to the S3 Storage.");
  }
